program main
   ...
   integer , parameter :: natom = 125  ! number of atoms
   integer , parameter :: n = natom * 3  ! number of freedom
   real*8, parameter :: m(n) = 1837.0d0 * 2.0d0  ! mass
   ...
   real*8, parameter :: omegaj(0:P-1) = [0d0,(1d0/betap/hbar*2*sin(count*pi/P),count=1,P-1)]
   real*8, parameter :: gamad = 1d0
   real*8, parameter :: lgam(0:P-1) = [0.017d0,(omegaj(count),count=1,P-1)]
   real*8, parameter :: lambda = 0.5d0
   real*8, parameter :: lgamad(0:P-1) = [0d0,(omegaj(count)/sqrt(gamad),count=1,P-1)]*lambda*2
   real*8, parameter :: dt = 40d0
   real*8, parameter :: dtr = dt*sqrt(gamad)
   real*8 :: mqj(n,0:P-1) = reshape([(m,count=0,P-1)],[n,P])
   real*8 :: mqjad(n,0:P-1) = reshape([m,(m*gamad,count=1,P-1)],[n,P])
   real*8 :: mxj(n,P) = reshape([(m,count=0,P-1)],[n,P])
   real*8 :: c1(0:P-1), c2(0:P-1)
   real*8 :: c1ad(0:P-1), c2ad(0:P-1)
   integer, parameter :: eqstep = 0 ! steps for equilibrium
   ...
   do k=1, samples
      if(zmaster) then
         ...
         !-----------------initial condition-------------------
         write(cc,*) ninit + k
         open(13,file='init_'//trim(adjustl(cc))//'.dat')
         read(13,*) qj
         close(13)
         ...
         if(ftrj) then
            open(21,file='trj_'//trim(adjustl(cc))//'.dat')
            open(22,file='trjc_'//trim(adjustl(cc))//'.dat')
            open(23,file='vtherm_'//trim(adjustl(cc))//'.dat')
            open(24,file='mtherm_'//trim(adjustl(cc))//'.dat')
            open(31,file='trj_map_'//trim(adjustl(cc)))
         end if
      ...
      do i = 1, sampstep
         ...
         if(zmaster) then
            ...
            if(mod(i,nout) .eq. 0) then
               hes_rec(:,:,-navg:navg-1) = hes_rec(:,:,-navg+1:navg) !?navg是0？
            ...
            if(i .eq. nout*(2*navg+1)) then
               !xphy = xj(:,1)
               xc = qj(:,0)/sqrt(dble(P)) ! read cent
               pc = pqj(:,0)/sqrt(dble(P))

               avghes = sum(hes_rec,3)/(navg*2+1) ! evaluate hes
               eigvec = avghes
               CALL dsyev('V','L',n,eigvec,n,eigval,work,3*n-1,ierr) ! diag
               if(ierr/=0) then
               ...
               if(ftrj) then
                  ...
                  write(22,*) xc, pc
                  write(23,*) vtherm
                  write(24,*) mtherm_diag
               end if
            end if

            if(mod(i,nout).eq.0 .and. i.gt.nout*(2*navg+1)) then

               do idof = 1, n ! evolve til
                  tmp1 = xtil(idof)*cos(omec(idof)*dt) + ptil(idof)*sin(omec(idof)*dt)
                  ptil(idof) = -xtil(idof)*sin(omec(idof)*dt) + ptil(idof)*cos(omec(idof)*dt)
                  xtil(idof) = tmp1
               end do

               xc = qj(:,0) / sqrt(dble(P))
               pc = pqj(:,0) / sqrt(dble(P)) ! evolve cent
               avghes = sum(hes_rec,3)/(2*navg+1) ! evaluate hes
               eigvec = avghes
               call dsyev('V','L',n,eigvec,n,eigval,work,3*n-1,ierr) ! diag
               if(ierr/=0) then
                  write(*,*) 'Diagonalization fails!'
               ...
               call dgemv('T',n,n,1d0,eigvec,n,ptmp,1,0d0,ptil,1)
               eigvec_old = eigvec
               ...
               do idof = 1, natom
                  mtherm_diag(:,:,idof) = mtherm(3*idof-2:3*idof,3*idof-2:3*idof) ! get mdiag
               end do
               call dsymv('U',n,1d0,mtherm,n,pphy,1,0d0,vtherm,1) ! get vtherm
...
contains
   ...
   subroutine CONS_TRPMDCOR(dtime) !质心不动 normalmode变换trpmd
      implicit none
   ...
   subroutine NMPIMD(pqj,qj,xj,fqj,fxj) ! NMPIMD 质心会动
      implicit none
   ...
   subroutine PACMD(pqj,qj,xj,fqj,fxj) ! PACMD 质心动 去掉振转 gamad?
      implicit none
      real*8, intent(inout) :: pqj(n,0:P-1), qj(n,0:P-1)
      real*8, intent(inout) :: xj(n,P), fqj(n,0:P-1), fxj(n,P)
