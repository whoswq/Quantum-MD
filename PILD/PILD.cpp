...
const int PIMD_steps = 100000; // 选取初始值时PIMD演化的步数
const int n_atom = 1;

double M_thermal[3][3] = {{48006.248658, 119.025604, -164.144211},
                          {119.025604, 2458.158386, -0.675177},
                          {-164.144211, -0.675177, 2458.392372}};
double M_thermal_inv[3][3] = {{2.08378786e-05, -1.00860136e-06, 1.39104571e-06},
                              {-1.00860136e-06, 4.06857461e-04, 4.43967884e-08},
                              {1.39104571e-06, 4.43967884e-08, 4.06862783e-04}};
double M_thermal_M_inv[3][3] = {{2.77788139e+01, 6.88741611e-02, -9.49820413e-02},
                                {6.88741611e-02, 1.42241325e+00, -3.90691145e-04},
                                {-9.49820413e-02, -3.90691145e-04, 1.42254865e+00}};
double M_M_thermal_inv[3][3] = {{3.60111985e-02, -1.74302502e-03, 2.40395023e-03},
                                {-1.74302502e-03, 7.03114989e-01, 7.67247756e-05},
                                {2.40395023e-03, 7.67247756e-05, 7.03124186e-01}};
...
// 定义一些与Morse势有关的常数
/* 参考2011年刘老师的文章
Insights in quantum dynamical effects in the infrared spectroscopy of liquid water from a
semiclassical study with an ab initio-based flexible and polarizable force field
                     ...
                     double (&p_init_array)[n_traj][N])
{
  ...
  for (int k = 0; k < N; k++)
  {
    for (int j = 0; j < n_traj; j++)
    {
      p_init_array[j][k] = para * distribution(generator) / sqrt(M_thermal_M_inv[k][k]);
    }
  }
}
...
int main()
{
  ...
  */
  ...
  cout << "the setting temperature is " << tem << endl;
  cout << "total steps is " << PIMD_steps << endl;

  for (int i = 0; i < PIMD_steps; i++)
  {
    BAOAB(x_array, p_array, nabla_Morse_potential, beta, omega_P, c1_PILD,
  ...
  cout << "and the setting temperature is " << tem << endl;
  /*
  ...
  for (int k = 0; k < n_traj; k++)
  {
    ...
    for (int j = 0; j < steps; j++)
    {
      ...
      BAOAB_PILD(x_array, p_array, nabla_Morse_potential, beta, omega_ad,
